...
树$\to$图论/树形$dp$/$DSU\ On\ Tree$

字符串$\to$字符串相关

$n$很小$\to$状压

$n^2$能过$\to$$O(n^2)\ dp$/搜索

$n\in [1e5,5e5)$$\to$$O(Nlog N)$(暴力($dp$/搜索)+数据结构优化($st$表/线段树/树状数组)/图论)/$O(n\sqrt{n})$相关算法(分块/莫队/$sqrt(n)$-$trick$套路题)

$n>1e6$$\to$线性算法($dfs$+优化/$bfs$)/卡常+$O(Nlog N)$算法(见上)
...
```c++
int get_tree(int l1, int r1, int l2, int r2) {//当前子树，a中[l1,r1]代表当前子树中序，b中[l2,r2]代表当前子树前/后序
	if(l1 > r1 || l2 > r2) return -1;//不合法，返回-1
	int root = b[r2], x = pos[root];//root:当前子树的根，x:当前root在a中的位置
	int y = (x - l1) + l2 - 1;//(x-l1)意思是，左子树大小，这样就能快速求出b中左右子树分界点在哪
    //b中[l2,y]是这颗子树的左子树区间，[y+1,r2-1]是右子树区间（注意r2是根节点所以不计入）
    //而a中[l1,x-1]是左子树，[x+1,r1]是右子树
...
## 15. 2-sat(并查集/$tarjan$，$m$对$bool$关系)
...
## 6.构造**排列**$a$，有$(\Pi_{i=1}^xa_i) \%\ n$在$x$不同时互不相同
...
## 8.排列 A 变成排列 B（相邻交换）的最小次数

将 $B$ 重排为 $1,2,...n$，对应的 $A$ 也要重排
...
如果真要用区间修改，单点查询（**注意，在线查询才需要用树状数组，在线修改+离线查询只需要差分+前缀即可**）
...
**如果初始值不为零，即选一些数和某个固定的数异或求最小**，那就要从高位到低位进行不断xor，比如$now$在某一位为1且这一位$p[i]$不为零，那么$now^=p[i]$即可
...
求某个数n<=$10^{28}$的欧拉函数，可以用以下代码求：

```c++
int phi = n;
for(p of n) {//p是n的质因数，p可以通过 Pollard–Rho 算法快速得到
    phi = phi / p * (p - 1);
}
```
...
当你需要 $O(1)$ 查询 $LCA$ 的时候，那传统的做法就不可取了，需要用到其他做法

```c++
const int N = 1e5 + 5, LOG = 20;
vector<int> g[N];
int first[N], euler[N * 2], dep[N * 2], st[N * 2][LOG], tot;
...
$tot$ 用来记录欧拉序（从 $v$ 返回 $u$ 的时候也要记录当前经过了 $u$，见代码中 $A*$​）

每个 $tot$ 要记录经过了哪个点，深度是多少

用 $first$ 记录第一次经过 $u$ 的时候 $tot$ 是多少

那么假设求 $u, v$ 的 $lca$， $[first_u, first_v]$ 这个区间（欧拉序区间）对应的深度最小的点即为它们的 $lca$

因为在这个欧拉序区间内只有可能经过 $lca(u,v)$ 的子树，那么对应的深度最小的当然就是 $lca(u,v)$ 了

用 $RMQ$ 预处理一下即可

#### 3.4.2 分块

如果你的树满足以下几个条件，那么就必须用分块来做 $LCA$

1. 满足 $fa_i<i$，即父亲节点编号一定小于自身编号
2. 每次都需要修改 $[l,r]$ 这个区间内的点的父亲节点编号
3. 需要求两个点之间的 $LCA$

时间复杂度：$O((n+q)\sqrt{n})$​

##### 例题：[[Ynoi2007] rfplca](https://www.luogu.com.cn/problem/P7446)

题目：

操作 1: 给定区间 [*l*,*r*]，将这个区间的 $fa_i=max(fa_i−x,1)$
操作 2: 求两点最近公共祖先

具体做法：

$[1,\sqrt{n}],[\sqrt{n}+1, 2\sqrt{n}]...$ 这样来划分区间（节点编号）

记录每个点的 $fa$ 和 $pa$，代表父亲节点和第一个编号不在块内的祖先节点的编号

当块内所有节点的 $fa$ 都不在块内那就可以暴力重构这个块了，可以发现最多暴力重构 $\sqrt{n}$ 次这个块内所有节点的 $fa$ 都一定不会在这个块内

如果 $fa$ 不在块内了就直接记录 $val$​ 代表这个块内所有点的 fa 需要整体向前多少即可

（**因为本质就是让求LCA的时候能一块一块的跳，如果fa已经在块外了那也就没必要继续单点维护了，直接区间维护即可**）

这样维护好之后，求 $LCA$ 的时候，每次如果 $pa_u\ne pa_v$ 那谁的 $pa$ 大谁就等于它的 $pa$，这样每次能跳至少 $\sqrt{n}$ 个点

$pa_u = pa_v$ 的时候就一步一步向上跳就行了（说明在同一块内），那么也是最多跳 $\sqrt{n}$ 次，所以查询时间复杂度是 $O(\sqrt{n})$，修改的总时间复杂度最高是 $O(n\sqrt{n})$
...
//getfa和getpa中，如果对应的区块重构次数大于sqrt(n)了，那pa和fa就无效了，直接用fa[x]-val[id[x]]即可
in int lca(int u, int v) {
	while(true) {
		if(u == v) break;
		int pau = getpa(u), pav = getpa(v);
		if(pau != pav) {
			if(pau > pav) u = pau;//谁编号大谁跳
			else v = pav;
		}
		else {
			if(u > v) u = getfa(u);//谁编号大谁跳
			else v = getfa(v);
		}
	}
	return u;
